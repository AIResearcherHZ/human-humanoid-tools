...
BASE_TILT_DEG = math.degrees(math.atan2(0.3173, 0.94832))
...
def make_scene(tilt_deg, workdir):
    ...
    scene_name = f"_tiltscan_scene_{tilt_deg:.6f}.xml"
...
def build(tilt_deg, workdir):
    m = mujoco.MjModel.from_xml_path(make_scene(tilt_deg, workdir))
    m.opt.gravity[:] = 0
...
def evaluate(tilt_deg, ctrl_unit, workdir, nthread):
    m = build(tilt_deg, workdir)
    idx = ArmIndex(m)
    ...
    n_valid = len(manip)
    if n_valid == 0:
        raise RuntimeError(f"no valid sample at tilt {tilt_deg:.3f} deg")
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(HERE, "shoulder_tilt_scan.json"))
    args = ap.parse_args()
    ...
    workdir = tempfile.mkdtemp(dir=ASSET_DIR, prefix="_tiltscan_")
    cache = {}

    def run(tilt):
        key = round(float(tilt), 4)
        if key not in cache:
            row = evaluate(key, ctrl_unit, workdir, args.nthread)
            cache[key] = row
            print(
                f"tilt={key:6.2f}  valid={row['valid']:4d}  "
    ...
    print("\ntilt_deg   score    manip   accel   torque   reach   clearance")
    for r in rows:
        t = r["log_ratio"]
        print(
            f"{r['tilt_deg']:8.2f}  {r['score']:+.4f}  {t['manip']:+.3f}  "
            f"{t['accel']:+.3f}  {t['torque']:+.3f}  {t['reach']:+.3f}  {t['clearance']:+.3f}"
        )
    print(f"\nbaseline {baseline['tilt_deg']:.2f} deg (score 0 by definition)")
    print(f"best     {best['tilt_deg']:.2f} deg -> score {best['score']:+.4f}")

    with open(args.out, "w", encoding="utf-8") as f:
        json.dump(
            {
                "baseline_tilt_deg": BASE_TILT_DEG,
                "weights": weights,
